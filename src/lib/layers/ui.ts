...
export function drawExtendBar(
  ...
  context.imageSmoothingEnabled = false;
  context.drawImage(
    getRes("ui"), 522, 223, 10, 18,
    x, y, 20, 36
  );
  for (let i = 0; i < w; i++) {
    if (i === 0) {
      context.drawImage(
        getRes("ui"), 533, 223, 8, 18,
        x + 20 + i * 16, y, 16, 36
      );
      if (health > i) {
        context.drawImage(
          getRes("ui"), 592, 183 + color * 20, 8, 18,
          x + 20 + i * 16, y, 16, 36
        );
      }
    } else {
      context.drawImage(
        getRes("ui"), 542, 223, 8, 18,
        x + 20 + i * 16, y, 16, 36
      );
      if (health > i) {
        context.drawImage(
          getRes("ui"), 601, 183 + color * 20, 8, 18,
          x + 20 + i * 16, y, 16, 36
        );
      }
    }
  ...
  context.drawImage(
    getRes("ui"), 567, 182 + color * 20, 16, 20,
    x + 20 + w * 16, y - 2, 32, 40
  );
  context.restore();
}